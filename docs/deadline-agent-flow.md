...
`conflicting_report`. This one instruction is what separates 7.3 from 7.4
...
**Relative weekday resolution rule (explicit, in-prompt).** "Next Friday"
and "this Friday" are genuinely ambiguous in colloquial English, and that
ambiguity is a realistic source of contradictions in real deadline chatter
— so it's resolved by a fixed rule stated directly in the prompt, not left
to the model's judgment:
- `"this <weekday>"` → the nearest upcoming occurrence of that weekday
  within the current week (relative to the message's `received_at`).
- `"next <weekday>"` → the occurrence *after* that — skip the nearest
  upcoming one, land on the following week's.

Both `due_date` and a `date_resolution_note` (what phrase was resolved and
against what anchor date) are returned by the model so a human can audit the
resolution later — this is what makes §7.4 below explainable rather than a
black box.
...
Applying the resolution rule from §3: the nearest upcoming Friday relative
to 08-24 is `2026-08-28` — but "next Friday" explicitly skips that one and
lands on the Friday of the week after, `2026-09-04`. That's a full week
later than the stored `t1.due_date` (`2026-08-28`), and message A contains
no override language ("not", "actually", "correction") — it's just stating
a date, unaware it disagrees with what's on file. So Gemini returns
`relation = conflicting_report`, `due_date = 2026-09-04`, with
`reasoning: "message states a date one week after the stored due_date,
with no indication it is correcting a prior value."`

**Backend action on message A:**
- `tasks.due_date` is **left at `2026-08-28`** — never overwritten by a
  `conflicting_report`.
- A new `task_versions` row is inserted: `due_date = 2026-09-04`,
  `source_message_id = <message A>`, `reason = conflicting_report`.
- `tasks.status` is set to `needs_confirmation`.

**Message B** — email, Tuesday 08-25, 6:40pm:
> "OS lab submission deadline: this Friday"

"This Friday" relative to 08-25 resolves to `2026-08-28` — which matches
the *stored* `t1.due_date` exactly (the field was never touched by message
A). So `relation = confirmation`, not a correction of A and not a new
conflict of its own — it simply re-affirms the value that's already on
file. This is the realistic trap the example is built to show: two people
both said "Friday" in good faith on different days, one used "next" and one
used "this," and the two phrases resolve a full week apart — neither
message is wrong, neither is lying, they just don't know they disagree.

**Backend action on message B:**
- A `task_versions` row is appended: `due_date = 2026-08-28`,
  `source_message_id = <message B>`, `reason = confirmation`.
- `tasks.status` is **left at `needs_confirmation`.** A confirmation of the
  original value doesn't clear a pending contradiction on its own — there's
  still an unresolved `2026-09-04` claim on record from message A, and
  picking a winner by "most recent message wins" or "majority of
  messages wins" would be exactly the kind of silent guess the assignment
  says never to make. The status only returns to `confirmed` via an
  explicit correction message that resolves the ambiguity outright (e.g.
  "confirmed: OS lab is the 28th, ignore the other date going around").
...
| `2026-09-04` | conflicting_report | message A |
| `2026-08-28` | confirmation | message B |

The "needs confirmation" query surfaces this task with both `2026-08-28`
and `2026-09-04` visible (join `tasks` to its `task_versions`), so the
student sees exactly which sources said what, rather than the agent
